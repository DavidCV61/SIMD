...
bool CalcMinMaxU8_Cpp(uint8_t *x_min, uint8_t *x_max, const uint8_t *x,
                      size_t n) {
  if (n == 0 || ((n % 16) != 0))
    return false;
...
bool CalcMinMaxU8_Iavx(uint8_t *x_min, uint8_t *x_max, const uint8_t *x,
                       size_t n) {
  if (n == 0 || ((n % 16) != 0))
    return false;
  if (!AlignedMem::IsAligned(x, 16))
  ...
  // Reducir min_vals al valor mínimo final
  ...
  *x_min = (uint8_t)_mm_extract_epi8(vals_r4, 0);
  ...
  // Reducir max_vals al valor máximo final
