...
public:
  enum class EtUnit { NanoSec, MicroSec, MilliSec, Sec };
  ...
  BmThreadTimer(size_t num_iter, size_t num_alg) {
    ...
    std::vector<std::chrono::high_resolution_clock::time_point> start_time(
        num_iter * num_alg);
    std::vector<std::chrono::high_resolution_clock::time_point> stop_time(
        num_iter * num_alg);
  ...
  static std::string BuildCsvFilenameString(const std::string &prefix,
                                            const std::string &base_name1,
                                            const std::string &base_name2) {
  ...
  void SaveElapsedTimes(const std::string &fn, EtUnit et_unit, int p) {
    ...
    if (ofs.is_open()) {
        ...
        for (size_t j = 0; j < m_NumAlg; j++) {
          ...
          default:
            throw std::runtime_error(
                "BmThreadTimer::SaveElapsedTimes() - Invalid EtUint");
          }
      ...
      throw std::runtime_error(
          "BmThreadTimer::SaveElapsedTimes() - File open error");
  }

  void Start(size_t iter_id, size_t alg_id) {
    m_StartTime[iter_id * m_NumAlg + alg_id] =
        std::chrono::high_resolution_clock::now();
  }

  void Stop(size_t iter_id, size_t alg_id) {
    m_StopTime[iter_id * m_NumAlg + alg_id] =
        std::chrono::high_resolution_clock::now();
  }
};
